				    ...
				    inout vec3 vColorOut)
{
	...
	float fSpotT = clamp((rho - vSpotParams.y) * vSpotParams.z, 0.0, 1.0);
				    ...
				    inout vec3 vColorOut)
{
	...
	for(int i = int(lightIndexLimit.x); i <= toIndex; ++i)
	{
		...
		vec4 dat1 = textureLod(dataTexture, vec2(widthOffset,heightCoord), 0);
		
		vec3 vLightView  = dat1.xyz - vViewPos;
		float fLightDist = length(vLightView);
		if (fLightDist * dat1.w < 1)
		{
			vec4 dat2 = textureLod(dataTexture, vec2(widthOffset + invWidth,heightCoord),0);
			vec4 dat3 = textureLod(dataTexture, vec2(widthOffset + invWidth * 2,heightCoord),0);
				    ...
				    inout vec3 vColorOut)
{
    ...
    vec2 debugColors = vColorOut.xy * 0.5 + ((mod(floor(indexes.xy),2) == vec2(0)) ? 0.1 : 0.2);
