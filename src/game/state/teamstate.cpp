...
//    while (i<gamePlayerInstance.size())
    ...
    {            
        if (GPIIT.second->getTeamID() == 0)
        {
            logMsg("gamePlayerInstance Team ID = " +convert->toString(GPIIT.second->getTeamID()) + "PLAYERNAME == " +GPIIT.second->getFirstName() +" " +GPIIT.second->getLastName());
        }
//        ++i;
    }
    exit(0);
